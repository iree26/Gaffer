...
_DEV_STORE_PATH = os.path.join(_DEV_STORE_DIR, "dev_memory.json")
...
class WalrusMemory:
    ...
    def _dev_read(self, user_id: str) -> dict | None:
    ...
    def _prod_read(self, user_id: str) -> dict | None:
        try:
    ...
    def _prod_delete(self, user_id: str) -> bool:
        try:
            with httpx.Client(timeout=10) as client:
                resp = client.delete(
                    f"{self.endpoint}/v1/memory/{user_id}",
            ...
            logger.error("PROD delete error: %s", e)
            return False

    # ── Public API ────────────────────────────────────────────────

    def read(self, user_id: str) -> dict | None:
        if not user_id:
            return None
        if self._dev_mode:
