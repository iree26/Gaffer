...
// index 0 of each group = the seeded favourite, so the gaffer can judge bold vs safe.
const GROUPS = [
...
// Stand-in for POST /agent/verdict — reads the whole card and reacts in the right voice.
function generateVerdict(picks, expertise) {
  ...
  return { headline, rows: [bold, safe, disagree], closing };
}
...
export default function PredictionScreen() {
  ...
  const [phase, setPhase] = useState(null);   // null | "thinking" | "reveal"
    ...
    <div className="gaffer-app">
      ...
      <style>{`
        ...
        .card{ border:1px solid var(--line); border-radius:18px; padding:1rem; background:rgba(255,255,255,.82);
          ...
          animation:cardIn .6s both cubic-bezier(.2,.8,.25,1); transition:box-shadow .22s ease, transform .22s ease; }
        ...
        .verdict{ width:100%; max-width:560px; text-align:center; background:rgba(255,255,255,.94); border:1px solid var(--line);
          border-radius:26px; padding:clamp(1.5rem,5vw,2.4rem); box-shadow:0 30px 90px rgba(7,94,50,.22);
          animation:vpop .5s cubic-bezier(.2,.8,.25,1); }
        ...
        .v-headline{ font-family:var(--display); font-weight:800; font-size:clamp(1.8rem,6vw,2.6rem); letter-spacing:-.02em;
          margin:.3rem 0 1.3rem; color:var(--ink); }
        ...
        .v-closing{ margin:1.2rem 0 0; padding:.9rem 1rem; background:rgba(22,180,95,.08); border-radius:14px;
          color:var(--deep); font-weight:600; line-height:1.45; font-size:.92rem; text-align:left;
          opacity:0; animation:vrow .6s .85s forwards; }
        ...
        .v-actions{ display:flex; gap:.8rem; justify-content:center; margin-top:1.4rem; flex-wrap:wrap;
          opacity:0; animation:vrow .6s 1.05s forwards; }
        ...
        @media (prefers-reduced-motion: reduce){
          ...
          .team,.team::before,.bar-fill{ transition:none !important; } .v-ball{ animation:none !important; }
        }
      `}</style>
      ...
      <div className="topbar">
        <div className="toprow">
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'1.2rem' }}>
                ...
                <span style={{ fontWeight:700, fontSize:'.85rem', color:'var(--bright)', cursor:'pointer' }}
                     onClick={() => navigate('/leaderboard')}>Table →</span>
         </div>
       </div>
